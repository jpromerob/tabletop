...
if __name__ == '__main__':
    ...
    common_cnn_pop = p.Population(OUT_WIDTH * OUT_HEIGHT, celltype(**s_cell_params),
                            structure=p.Grid2D(OUT_WIDTH / OUT_HEIGHT), label=S_CNN_POP_LABEL)


    # Projection from SPIF virtual to CNN populations
    p.Projection(p_spif_virtual_a, common_cnn_pop, f_cnn_conn, p.Convolution())
    p.Projection(p_spif_virtual_a, common_cnn_pop, m_cnn_conn, p.Convolution())
    p.Projection(p_spif_virtual_a, common_cnn_pop, s_cnn_conn, p.Convolution())
    ...
    p.external_devices.activate_live_output_to(common_cnn_pop, spif_s_cnn_output)
