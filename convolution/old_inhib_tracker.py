...
def parse_args():
    ...
    parser.add_argument('-ps', '--port-s-cnn', type= int, help="Port Out (slow)", default=3334)
    parser.add_argument('-pm', '--port-m-cnn', type= int, help="Port Out (mixed)", default=3337)
...
if __name__ == '__main__':
    ...
    # Setting up mixed layer
    m_cnn_pop = p.Population(OUT_WIDTH * OUT_HEIGHT, celltype(**f_cell_params),
    ...
    exc_w = 40
    exc_syn = p.StaticSynapse(weight=exc_w, delay=0.1)
    inh_w = 20
    inh_syn = p.StaticSynapse(weight=inh_w, delay=0.1)

    conn_list = create_one_to_one_cnn_2d(OUT_WIDTH, OUT_HEIGHT, NPC_X, NPC_Y)
    cell_conn = p.FromListConnector(conn_list, safe=True) 
    p.Projection(s_cnn_pop, m_cnn_pop, p.OneToOneConnector(), receptor_type='excitatory', synapse_type=exc_syn)
    p.Projection(f_cnn_pop, m_cnn_pop, p.AllToAllConnector(), receptor_type='inhibitory', synapse_type=inh_syn)
