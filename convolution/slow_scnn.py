...
if __name__ == '__main__':
    ...
    WIDTH = dim.fl
    HEIGHT = dim.fw
    OUT_WIDTH = WIDTH-len(s_kernel)+1
    OUT_HEIGHT = HEIGHT-len(s_kernel)+1
    ...
    try:
        ...
        print(f"\tInput Through SPIF @ {SPIF_IP_L} (Chip {CHIP_I})")
        print(f"\tNPC: {NPC_X} x {NPC_Y}")
        print(f"\tOutput {OUT_WIDTH} x {OUT_HEIGHT}")
        print(f"\tKernel Size: {len(s_kernel)}")
        print(f"\tKernel Sum: {abs(round(np.sum(s_kernel),3))}")
