...
def plot_configuration(fig, ax, coor_frame, lfj_pt, rfj_pt, o_x, o_y, l_0, l_t, w_t):
        ...
        plt.savefig('images/tabletop_'+ pt_names[idx] +'.png', dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    pt_array = []
    for i in range(w_t):
        pt_names.append(str(int(i)))
        pt_array.append((i-o_x, o_y))
    for i in range(int(l_t/2)):
        pt_names.append(str(int(i+w_t)))
        pt_array.append((l_0+o_x, o_y+i))
    for i in range(w_t):
        pt_names.append(str(int(i+w_t+l_t/2)))
        pt_array.append((l_0+o_x-i, l_t/2+o_y))
    for i in range(int(l_t/2)):
        pt_names.append(str(int(i+l_t/2+2*w_t)))
        pt_array.append((-o_x, l_t/2+o_y-i))
    ...
    for idx, pt in enumerate(pt_array):
        ...
        fig, ax = plt.subplots()
        plot_configuration(fig, ax, 'motor_middle', lfj_pt, rfj_pt, o_x, o_y, l_0, l_t, w_t)
        plt.close()
    # pdb.set_trace()