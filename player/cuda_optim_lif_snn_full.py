...
def process_data(out_x_dev, out_y_dev, res_x, res_y, sock):


    idx_x_dev = torch.where(out_x_dev[0] > 0.5)[0]
    idx_y_dev = torch.where(out_y_dev[0] > 0.5)[0]


    try:
        if len(idx_x_dev)>0 and len(idx_y_dev) > 0:

            x_coord = torch.mean(idx_x_dev.float()).item()
            y_coord = torch.mean(idx_y_dev.float()).item()
    ...
    with torch.inference_mode():
        ...
        with aestream.UDPInput((res_y, res_x), device = 'cpu', port=args.port) as stream1:
            ...
            while True:
                ...
                process_data(out_x_dev, out_y_dev, res_x, res_y, sock)
