...
PORT_UDP_PADDLE_DESIRED_2D = 6565  # Choose an appropriate port number
...
# Cell parameters
p=LIFBoxParameters(tau_mem_inv=torch.tensor(1000/1.0), #1ms
    ...
    def __init__(self, N, M): #N: x size, M: y size (from robot's perspective)
        ...
        for i in range(N):  # height
            for j in range(M):  # width
                ...
                if j <= mirror:
                    ...
                    # self.in_mid.append((j, i, j, int((i - N / 2) * (M-j) / (M-middle_right) + N / 2)))
        ...
        for i in range(pts):
            try:
                ...
                self.middle_layer.weight.data[middle_index, input_index] = 120
...
def parse_args():
    ...
    parser.add_argument('-p', '--port', type= int, help="Port for events", default=PORT_UDP_PADDLE_DESIRED_2D)
