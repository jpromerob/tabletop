...
class ControlNet(torch.nn.Module):
    def __init__(self, N, M): #N: x size, M: y size (from robot's perspective)
        ...
        for i in range(pts):
            try:
                ...
                self.output_layer_x.weight.data[x_index, input_index] = 0.2
                self.output_layer_y.weight.data[y_index, input_index] = 0.2
            except:
                pdb.set_trace()
    ...
    def forward(self, x):
        ...
        middle_layer = self.mid_cell(self.middle_layer(input_layer))
        output_N = self.out_x_cell(self.output_layer_x(middle_layer))
        output_M = self.out_y_cell(self.output_layer_y(middle_layer))
        middle_layer = middle_layer.view(-1, self.N, self.M)
        return middle_layer, output_N, output_M
