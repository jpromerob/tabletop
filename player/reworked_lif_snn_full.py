    ...
    with torch.inference_mode():
        ...
        with aestream.UDPInput((res_y, res_x), device = 'cpu', port=args.port) as stream1:
            ...
            while True:
                ...
                after_model.append(time.time() - start_time)

                out_x = out_x_dev.squeeze()
                out_y = out_y_dev.squeeze()

                before_where.append(time.time() - start_time)
                idx_x = torch.where(out_x > 0.5)
                idx_y = torch.where(out_y > 0.5)
                after_where.append(time.time() - start_time)


                try:
                    if len(idx_x[0]) > 0 and len(idx_y[0]) > 0:
                        # Calculate mean indices
                        x_coord = idx_x[0].float().mean().item() 
                        y_coord = idx_y[0].float().mean().item() 
                ...
                before_sock.append(time.time() - start_time)
                ...
                before_end.append(time.time() - start_time)
                if frame_counter % 100 == 0:
                    if len(before_end) > 0:

                        after_model_ms = 1000 * sum(after_model) / len(after_model)
                        print("Time elapsed after model:", after_model_ms, "ms")
                        after_model = []

                        before_where_ms = 1000 * sum(before_where) / len(before_where)
                        print("Time elapsed before_where:", before_where_ms, "ms")
                        before_where = []

                        after_where_ms = 1000 * sum(after_where) / len(after_where)
                        print("Time elapsed after_where:", after_where_ms, "ms")
                        after_where = []

                        before_sock_ms = 1000 * sum(before_sock) / len(before_sock)
                        print("Time elapsed before_sock:", before_sock_ms, "ms")
                        before_sock = []

                        before_end_ms = 1000 * sum(before_end) / len(before_end)
                        print("Time elapsed before end:", before_end_ms, "ms")
                        before_end = []
