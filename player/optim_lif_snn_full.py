...
def setParams(tau_mem):
    ...
    return LIFParameters(
        ...
        tau_syn_inv=torch.tensor(1000/1),
...
class ControlNet(torch.nn.Module):
    def __init__(self, in_x_sz, in_y_sz, mode): #x size, y size (from robot's perspective)
        ...
        self.f_conv_cell = LIFCell(p=setParams(1), dt=self.dt)
        self.v_f_conv = None
        ...
        self.s_conv_cell = LIFCell(p=setParams(64), dt=self.dt)
        self.v_s_conv = None
        ...
        self.mapper_cell = LIFCell(p=setParams(1), dt=self.dt)
        self.v_mapper = None
        ...
        self.out_x_cell = LIFCell(p=setParams(1), dt=self.dt)
        self.v_x_proj = None

        # Define Y-Projection Layer
        self.output_layer_y = torch.nn.Linear(self.mapper_size, self.y_proj_sz) 
        self.out_y_cell = LIFCell(p=setParams(1), dt=self.dt)
        self.v_y_proj = None
...
def parse_args():
    ...
    parser.add_argument('-t','--tracking', action='store_true', help='Send Tracking Data')
...
if __name__ == '__main__':
    ...
    nb_samples = 100
    t_scope = np.zeros((10, nb_samples))
    max_t_idx = 0
    sample_counter = 0
    t_labels = []
    ...
    with torch.inference_mode():
        ...
        with aestream.UDPInput((res_y, res_x), device = 'cpu', port=args.port) as stream1:
            ...
            while True:
                ...
                t_idx = 0
                if sample_counter+1 < nb_samples:
                    sample_counter += 1
                else:
                    print("\n\n")
                    for i in range(max_t_idx):
                        print(f"{t_labels[i]}: {round(1000*np.mean(t_scope[i,:]),1)} [ms]")
                    sample_counter = 0
                    t_labels = []
                ...
                if sample_counter:
                    t_labels.append("After AEStream Reading")
                t_scope[t_idx, sample_counter] = time.time()-t_start
                t_idx += 1
                ...
                if sample_counter:
                    t_labels.append("After Moving Data to GPU")
                t_scope[t_idx, sample_counter] = time.time()-t_start
                t_idx += 1
                ...
                if sample_counter:
                    t_labels.append("After Running Model")
                t_scope[t_idx, sample_counter] = time.time()-t_start
                t_idx += 1
                ...
                out_x_dev_squeezed = torch.squeeze(out_x_dev).cuda()
                out_y_dev_squeezed = torch.squeeze(out_y_dev).cuda()

                idx_x_dev = torch.nonzero(out_x_dev_squeezed > 0.5).cuda() #torch.where(out_x_dev[0] > 0.5)[0]
                idx_y_dev = torch.nonzero(out_y_dev_squeezed > 0.5).cuda() #torch.where(out_y_dev[0] > 0.5)[0]


                if sample_counter:
                    t_labels.append("Right Before transferring data from GPU to CPU")
                t_scope[t_idx, sample_counter] = time.time()-t_start
                t_idx += 1

                if idx_x_dev.numel() > 0 and idx_y_dev.numel() > 0:

                    # pdb.set_trace()

                    x_coord = torch.mean(idx_x_dev.float()).cpu()
                    y_coord = torch.mean(idx_y_dev.float()).cpu()
                ...
                if sample_counter:
                    t_labels.append("Right After transferring data from GPU to CPU")
                t_scope[t_idx, sample_counter] = time.time()-t_start
                t_idx += 1
                ...
                if sample_counter:
                    t_labels.append("Right After Sending XY (if ...)")
                t_scope[t_idx, sample_counter] = time.time()-t_start
                t_idx += 1
                ...
                # if args.visualize and args.tracking:
