...
def create_inh_xy_conn_list(width, height):
    delay = 1 # [ms]
    weight = W_INHIBITION
    conn_inh_xy_list = []
...
if __name__ == '__main__':
    ...
    xy_pop = p.Population(WIDTH + HEIGHT, p.IF_curr_exp(), label=MID_POP_LABEL)
    conn_2D_1D_list = create_2D_to_1D_conn_list(WIDTH, HEIGHT)
    cell_conn_2D_1D = p.FromListConnector(conn_2D_1D_list, safe=True)     
    p.Projection(spif_retina, xy_pop, cell_conn_2D_1D, receptor_type='excitatory')


    inhibitory_pop = p.Population(2, p.IF_curr_exp(), label=GIN_POP_LABEL)    

    # Excitatory connections from xy_pop inhibitory_pop
    conn_xy_inh = create_xy_inh_conn_list(WIDTH, HEIGHT)
    cell_conn_xy_inh = p.FromListConnector(conn_xy_inh, safe=True)     
    p.Projection(xy_pop, inhibitory_pop, cell_conn_xy_inh, receptor_type='excitatory')

    # Inhibitory connections from inhibitory_pop to xy_pop
    conn_inh_xy = create_inh_xy_conn_list(WIDTH, HEIGHT)
    cell_conn_inh_xy = p.FromListConnector(conn_inh_xy, safe=True)     
    p.Projection(inhibitory_pop, xy_pop, cell_conn_inh_xy, receptor_type='inhibitory')
    
    spif_output = p.Population(None, p_spif_out, label=OUT_POP_LABEL)
    p.external_devices.activate_live_output_to(xy_pop, spif_output)
