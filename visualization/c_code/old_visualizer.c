...
void* renderMatrix(void* arg) {
    ...
    SDL_Window* window = SDL_CreateWindow("Air Hockey", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WINDOW_WIDTH*scale, WINDOW_HEIGHT*scale, SDL_WINDOW_SHOWN);
    ...
    while (1) {
        ...
        for (int y = 0; y < NB_ROWS; y++) {
            for (int x = 0; x < NB_COLS; x++) {
                int index = y*scale * NB_COLS + x*scale;
                int color = outputMatrix[x][y] ? 0xFF00FF00 : 0xFF000000; // Green for 1, transparent for 0
