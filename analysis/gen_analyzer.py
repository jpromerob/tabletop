...
def gen_ev_process(shared_data):
    ...
    if shared_data['gpu']:
        ...
        os.system(f"python3 ~/tabletop/generation/puck_generator.py -s {shared_data['sparsity']} -d {shared_data['delta']} -m circle -g")
    else:
        print("Launch event generation for SpiNNaker")
        os.system(f"python3 ~/tabletop/generation/puck_generator.py -s {shared_data['sparsity']} -d {shared_data['delta']} -m circle")
